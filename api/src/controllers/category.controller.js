...
async function createCategory(req, res) {
  try {
    ...
    if (!name) throw new StatusError(400, 'Category name is required');
    if (parentId && !(await Category.findById(parentId)))
      throw new StatusError(400, `Category with ID: ${parentId} does not exist`);
    if (!parentId && !req.file) throw new StatusError(400, "Image with 'image' key is required");
    ...
    if (!parentId && req.file) {
      ...
      if (parentId) await Category.updateOne({ _id: parentId }, { $push: { children: item } });
      res.json(item);
    }
  } catch (e) {
    errorHandler(res, e);
...
async function getCategoryById(req, res) {
  try {
    const { id } = req.params;
    if (!(await Category.findById(id))) throw new StatusError(400, `Category with ID: ${id} does not exist`);
...
async function deleteCategory(req, res) {
  try {
    const { id } = req.params;
    if (!(await Category.findById(id))) throw new StatusError(400, `Category with ID: ${id} does not exist`);
    const response = await Promise.all([Category.deleteOne({ _id: id }), Category.deleteMany({ parentId: id })]);
    res.json(response);
  } catch (err) {
    errorHandler(res, err);
  }
